...
class OrderSpecTest {
	...
	@Test
	void testAdd() throws PizzaIdNotFoundException, InvalidSizeException {
		
		Customer c1 = new Customer(101, "John", Long.parseLong("8945542130"), "john@gmail.com", "Los Angeles", "john", "john123");
		Coupan coupan = new Coupan(999, "FIRST_LOGIN_COUPAN", "FIRST_LONG","Apply first coupan");
		...
		Pizza pizza = pizzaRepo.viewPizza(1);
		Pizza pizza2 = pizzaRepo.viewPizza(2);
		...
		order1.setOrderId(1000001);
		order1.setOrderType("Veg Pizza Order");
		order1.setOrderDescription("order setting");
		order1.setCustomerId(c1);
		order1.setCoupan(coupan);
		order1.setTransactionMode("Cash");
		map.put(pizza, 2);
		map.put(pizza2, 3);
		order1.setMapPizzaQuantity(map);
		orderList = orderRepoTest.caluculateTotal("Medium", 2, list1);
